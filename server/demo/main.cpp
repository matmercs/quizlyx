...
public:
  Demo() :
      session_manager_(quiz_registry_, broadcast_sink_, time_provider_),
...
private:
  ...
  void SimulateClient(int game_num) {
    ...
    // Create session — use promise/future to wait for result (Fix 4)
    ...
    DemoSessionInfo info = session_future.get();
    if (info.session_id.empty())
      return;

    // Join players
    std::vector<std::string> player_names = {"Alice", "Bob", "Charlie"};
    for (const auto& name : player_names) {
      command_queue_.Push(JoinPlayerCmd{.session_id = info.session_id,
                                        .pin = info.pin,
                                        .display_name = name + "_g" + std::to_string(game_num),
                                        .game_num = game_num});
    ...
    // Get player IDs from the session
    std::vector<std::string> player_ids;
    for (const auto& p : session->players) {
      if (p.role == domain::Role::Player) {
    ...
    for (size_t q = 0; q < quiz->questions.size(); ++q) {
      ...
      for (const auto& player_id : player_ids) {
        ...
        command_queue_.Push(SubmitAnswerCmd{
            .session_id = info.session_id, .player_id = player_id, .answer = answer, .game_num = game_num});
