...
Player* FindPlayer(Session& s, const std::string& player_id) {
  auto it = std::find_if(s.players.begin(), s.players.end(),
                         [&player_id](const Player& p) { return p.id == player_id; });
...
bool AddPlayer(Session& s, const Player& p) {
  if (s.state != SessionState::Lobby) return false;
  if (FindPlayer(s, p.id) != nullptr) return false;
...
bool StartGame(Session& s) {
  if (!CanStartGame(s)) return false;
...
bool CanSubmitAnswer(const Session& s, const std::string& player_id) {
  if (s.state != SessionState::Running) return false;
...
void RemovePlayer(Session& s, const std::string& player_id) {
  s.players.erase(
      std::remove_if(s.players.begin(), s.players.end(),
                     [&player_id](const Player& p) { return p.id == player_id; }),
      s.players.end());
}
